...
test("FormAssociatedFileElement has correct default properties", () => {
	expect(element.excludeAcceptAllOption).toBe(true)
...
test("FormAssociatedFileElement validates file types", async () => {
	...
	expect(element.files).toEqual([validFile])
	...
	expect(element.validationMessage).toBe("Please upload an accepted file type if you want to proceed.")
})
...
test("FormAssociatedFileElement validates file size", () => {
	...
	expect(element.validationMessage).toBe("Please upload an smaller file size if you want to proceed.")
	...
	expect(element.files).toEqual([smallFile])
	expect(element.validity.valid).toBe(true)
})
...
test("FormAssociatedFileElement handles multiple files", () => {
	...
	expect(element.files).toEqual([file1, file2])
	expect(element.validity.valid).toBe(true)
})
...
test("FormAssociatedFileElement detects files with missing types", () => {
	...
	expect(element.files).toEqual([fileWithMissingType])
	expect(element.validity.valid).toBe(true)
})
