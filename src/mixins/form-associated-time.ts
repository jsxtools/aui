...
export const FormAssociatedTimeMixin = <T extends CustomElementConstructor>(Element: T): T & FormAssociatedTimeMixin.Constructor =>
	class extends FormAssociatedMixin(Element) {
		constructor() {
			const host = super() as unknown as InstanceType<T> & FormAssociatedTimeMixin.Mixin
			const root = host.attachShadow({ delegatesFocus: true, mode: "open" })
